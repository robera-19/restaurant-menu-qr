...
const Categories = () => {
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        'Delete this category? Items will remain but lose this Catefory.',
      )
    )
      return;
    try {
          ...
          <div className="relative z-10">
            ...
            >
              <input
                ...
                placeholder="e.g. Pizzas, Burgers, Hot Drinks..."
                className="w-full border-none bg-slate-50 p-4 rounded-2xl outline-none ring-1 ring-slate-200 focus:ring-2 focus:ring-orange-500 transition-all font-medium text-slate-700 placeholder:text-slate-300"
              />
              <button
                disabled={isSubmitting}
                className="w-full sm:w-auto bg-orange-600 text-white px-6 sm:px-10 py-4 rounded-2xl font-black uppercase tracking-tighter hover:bg-orange-700 shadow-lg shadow-orange-600/20 transition-all active:scale-95 flex items-center justify-center min-w-[120px]"
              >
                {isSubmitting ? (
                  <Loader2 className="animate-spin" size={22} />
                ) : (
                  <>
                    <Plus size={22} className="mr-1" /> Add Category
                  </>
                )}
              </button>
            </form>
        ...
        <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] ml-1">
              Organized Categories ({categories?.length || 0})
            </h3>
            {loading && (
