...
import { Heart, Share2, AlertCircle, CheckCircle, Clock, Flame, Star, Minus, Plus, ChevronLeft } from 'lucide-react';
    ...
    >
      ...
      >
        <div className="relative">
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setIsLiked(!isLiked)} className="absolute top-4 right-4 bg-white/90 backdrop-blur rounded-full p-2">
        ...
        <div className="p-5">
          <div className="mb-4">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              {item.preparation_time && <span className="inline-flex items-center gap-1 text-gray-600 text-xs"><Clock size={12} /> {item.preparation_time} min</span>}
              {item.spicy_level > 0 && <span className="inline-flex items-center gap-1 text-orange-600 text-xs"><Flame size={12} /> {'🌶️'.repeat(item.spicy_level)}</span>}
              {item.rating && <span className="inline-flex items-center gap-1 text-yellow-600 text-xs"><Star size={12} /> {item.rating} ⭐</span>}
            </div>
          </div>
